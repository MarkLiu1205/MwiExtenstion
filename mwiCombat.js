...
(function () {
    ...
    const UI_TEXT = {
        ...
        zh: {
            button: "從主站匯入",
            waitingMainSite: "等待主站回應…",
            importingSimulator: "正在匯入到模擬器…",
            importSuccess: "匯入成功。",
            importFailed: "匯入失敗。",
            noMainSiteData: "未從主站收到可匯入的資料。請確認主站標籤頁已開啟並已登入。",
            simulatorImportFailed: "模擬器頁面未能完成匯入。",
            pageBridgeTimeout: "等待頁面橋接回應逾時。",
            mainSiteTabTimeout: "等待主站標籤頁回應逾時。",
            currentCharacterNotInitialized: "目前角色尚未初始化，請重新整理一次主站標籤頁。",
            unableToReadCurrentProfile: "無法讀取目前角色資料。",
    ...
    // 战斗(Combat)主导航项目用 aria-label="navigationBar.combat" 这个 i18n key 找，
    // 不受语系文字（戰鬥/战斗/Combat）影响，比文字比对稳定
    function findMainNavCombatItem() {
        const icon = document.querySelector('svg[aria-label="navigationBar.combat"]');
        return icon ? icon.closest(".NavigationBar_navigationLink__3eAHA") : null;
    }

    // 找一个结构最简单的主导航项目（没有经验条、没有展开箭头）当克隆样板，
    // 这样才能完整保留 .nav/.contentContainer/.textContainer 这层结构，跟其他主导航项目长得一样
    function findMainNavSimpleTemplate(excludeEl) {
        const candidates = document.querySelectorAll(".NavigationBar_navigationLink__3eAHA");
        for (const el of candidates) {
            if (el === excludeEl || el.id === MAIN_SITE_SHORTCUT_ID) {
                continue;
            }
            if (el.querySelector(".NavigationBar_experienceBar__2fo3Q, .NavigationBar_toggleContainer__2Gsp3")) {
                continue;
            }
            return el;
        }
    ...
    // 克隆样板时完整保留 .nav/.contentContainer/.textContainer 结构，只换图标 href 跟标签文字，
    // 这样跟其他主导航项目（耐力、智力...）外观完全一致，展开/收合时也沿用游戏原生的 CSS，不用自己再写一套响应式判断
    function createMainSiteShortcut(referenceItem, iconHref) {
        const shortcut = referenceItem.cloneNode(true);
        const preferredLanguage = resolveUiLanguage();

        shortcut.id = MAIN_SITE_SHORTCUT_ID;
        shortcut.setAttribute("data-mwi-tm-main-shortcut", "simulator");
        // 样板可能带有自己专属的 class（例如我的物品的 characterManagement、戰鬥的 active 状态），
        // 只保留通用的导航项目结构 class，避免被其他脚本认成那个特定项目
        shortcut.className = "NavigationBar_navigationLink__3eAHA";
        shortcut.removeAttribute("aria-current");
        // 样板可能是被其他脚本隐藏/标记过的项目，先清掉内联样式避免带过来
        shortcut.removeAttribute("style");
        ...
        // 样板可能带有等级数字、经验条、子技能列表，这些对捷径没有意义，清掉
        shortcut.querySelectorAll(
            ".NavigationBar_level__3C7eR, .NavigationBar_experienceBar__2fo3Q, .NavigationBar_subSkills__37qWb, .insertedSpan"
        ).forEach((element) => element.remove());

        const useEl = shortcut.querySelector("svg use");
        if (useEl && iconHref) {
            useEl.setAttribute("href", iconHref);
        }

        const label = shortcut.querySelector(".NavigationBar_label__1uH-y");
        if (label) {
    ...
    function mountMainSiteSimulatorShortcut() {
        ...
        const combatItem = findMainNavCombatItem();
        if (!combatItem || !combatItem.parentElement) {
            return;
        }

        const template = findMainNavSimpleTemplate(combatItem);
        if (!template) {
            return;
        }

        const combatIconUse = combatItem.querySelector("svg use");
        const iconHref = combatIconUse ? combatIconUse.getAttribute("href") : null;

        const shortcut = createMainSiteShortcut(template, iconHref);
        combatItem.parentElement.insertBefore(shortcut, combatItem.nextSibling);
    }
    ...
    function initSimulatorImportButton() {
        ...
        function formatTeamImportSummary(successCount, failureEntries = []) {
            ...
            const suffix = failedCount > 2
                ? (state.uiLanguage === "zh" ? `……另有 ${failedCount - 2} 個失敗` : `… +${failedCount - 2} more`)
                : "";
        ...
        async function importTeamMainSiteResponse(mainSiteResponse) {
            ...
            setStatus(state.uiLanguage === "zh" ? `匯入完成：${summary}` : `Import finished: ${summary}`, "success");
        }
